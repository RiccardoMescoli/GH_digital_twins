...
def _gh_block_dt_proxy_generator(block_id):
    dt_proxy = None
    dt_proxy = pyro.Proxy(
        f"PYROMETA:DT:GH_block,greenhouse:{block_id.value[0]},block:{block_id.value[1]}")
    print(dt_proxy)
    yield dt_proxy
...
def _initialize_graph_data(rem_source):
    ...
    try:
        ...
        print("ERROR: The given object lacks an attribute")
...
ns = pyro.locate_ns()
remote_source = None
remote_source = pyro.Proxy(f"PYROMETA:DT:GH_block,greenhouse:{BLOCK_ID_OPTIONS[0][0][0]},block:{BLOCK_ID_OPTIONS[0][0][1]}")
print(remote_source)
