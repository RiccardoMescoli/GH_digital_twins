...
from Pyro5.errors import CommunicationError, ProtocolError, NamingError
...
class GHBlockDT(object):
    ...
    @staticmethod # Utilizza questo nuovo metodo per determinare l'URI utilizzabile (utilizzabile solo su DT su cont.)
    def __get_uri(metadata_dict, key):
        uri = key
        metadata = metadata_dict[key][1]
        value = "docker:False"
        for val in metadata:
            if val.startswith("docker:"):
                value = val
        is_docker = True if value.split(":")[-1] == "True" else False

        if is_docker:
            for val in metadata:
                if val.startswith("docker_uri:"):
                    uri = val.split("uri:")[-1]
        return uri

    @staticmethod
    def __get_id_from_URI(URI):
        return URI.split("_")[-1].split("@")[0]

    @staticmethod
    def __get_docker_URI(metadata, name):
        URI = name
        for data in metadata[name][1]:
            if data.startswith("docker_uri"):
                URI = data.split("_uri:")[1]
        return URI
    ...
    def lookup_master(self, startup=False):
        ...
        peer_meta = pyro.locate_ns(NAMESERVER_HOSTNAME).yplookup(meta_all=[
                                             "DT:GH_block",
        ...
        if len(peer_list) > 0:
            ...
            while master_id is None and len(peer_list) > 0:
                ...
                try:
                    peer_net_id = self.__get_id_from_URI(proxy_name)
                    if peer_net_id != self.__network_id:
                        with pyro.Proxy(f"PYROMETA:{NETWORK_ID_METADATA_KEY+peer_net_id}@{NAMESERVER_HOSTNAME}") as proxy:
                            print(proxy)
        ...
        if no_peers:  # If this is the only digital twin of its kind it elects itself master
            ...
            try:
                with pyro.Proxy(f"PYROMETA:{NETWORK_ID_METADATA_KEY + master_id}@{NAMESERVER_HOSTNAME}") as master_proxy:
                    master_proxy.ping()
            except (CommunicationError, NamingError):
    ...
    def initiate_leader_election(self, initiator=False, startup=False):
        ...
        for proxy_name in peer_list:
            ...
            if not proxy_is_self:
                with pyro.Proxy(f"PYROMETA:{NETWORK_ID_METADATA_KEY+self.__get_id_from_URI(proxy_name)}@{NAMESERVER_HOSTNAME}") as proxy:
        ...
        for i in range(QUERY_FORWARDING_MAX_ATTEMPTS):
            try:
                ...
                if self.__is_master:  # In case the contacted node was actually a slave, lookup the real master
                    datalogger_names = list(pyro.locate_ns(NAMESERVER_HOSTNAME).yplookup(meta_all=["datalogger"]).keys())
                    ...
                    proxy = pyro.Proxy(f"PYROMETA:{NETWORK_ID_METADATA_KEY + self.__master_id}@{NAMESERVER_HOSTNAME}")
                error = e
            except (CommunicationError, NamingError) as e:  # In case the master goes down, initiate a new leader election
    ...
    def get_sensor_log(self, feed_id, days=0.0, hours=0.0, minutes=0.0, seconds=0.0):
        # TODO: Usa il metadato "datalogger" direttamente alla creazione del proxy per trovare il logger
        datalogger_names = list(pyro.locate_ns(NAMESERVER_HOSTNAME).yplookup(meta_all=["datalogger"]).keys())
...
daemon = pyro.Daemon(host=get_ip_address(),
                     port=DT_PORT,
                     nathost=os.getenv(HOST_IP_ENV_VAR),
                     natport=int(os.getenv(CONTAINER_PORT_ENV_VAR))
                     )
ns = pyro.locate_ns(NAMESERVER_HOSTNAME)
print("-- NS found!")
gh_block_obj = GHBlockDT(GREENHOUSE_ID, BLOCK_ID)
global_uri = daemon.register(gh_block_obj)

# The network ID of the digital twin is extracted directly from its URI
net_id = str(global_uri).split('_')[1].split('@')[0]
gh_block_obj.set_network_id(network_id=net_id)

docker_uri_list = str(global_uri).split("@")
# global_uri_list[0] += "-Docker"
docker_uri_list[1] = (get_ip_address() + ":" + str(DT_PORT))
docker_uri = pyro.URI("@".join(docker_uri_list))
print(docker_uri)
print(global_uri)

net_id_META = NETWORK_ID_METADATA_KEY + net_id
GH_BLOCK_METADATA.add(net_id_META)
GH_BLOCK_METADATA.add("docker_uri:" + str(docker_uri))
GH_BLOCK_METADATA.add("docker:True")
# GH_BLOCK_METADATA_DOCKER.add(net_id_META)

ns.register(str(global_uri), docker_uri, metadata=GH_BLOCK_METADATA)
print(" -- Registered on NS")
gh_block_obj.lookup_master(startup=True)
print(" -- Lookup ended")
# ns.register(str(docker_uri), docker_uri, metadata=GH_BLOCK_METADATA_DOCKER)
# ns.register(str(docker_uri), docker_uri, metadata=GH_BLOCK_METADATA)
print(f"Greenhouse block {global_uri}: READY")
# print(f"Greenhouse block {docker_uri}: READY")
print(f"Docker URI: {docker_uri}")
print(f"Network ID: {net_id}\n")
print(f"Nameserver: {ns.list()}")
daemon.requestLoop()
